...
interface WalletResponse { data: { data: Wallet[] } }

interface Itinerary { id: number; name: string }
interface ItineraryResponse { data: Itinerary[] }
...
interface ExpenditureResponse { data: { data: ExpenditureItem[] } }

/* ─── Section Label ──────────────────────────────────────── */
const SectionLabel = React.memo(({ icon, label }: { icon: keyof typeof Ionicons.glyphMap; label: string }) => (
...
}: { items: Itinerary[]; selected: number | null; onSelect: (id: number) => void }) => (
            ...
            return (
                ...
                >
                    <View style={styles.selectItemInner}>
                        <View style={[styles.selectRadio, active && styles.selectRadioActive]}>
                            {active && <View style={styles.selectRadioDot} />}
                        </View>
                        <Text
                            style={[styles.selectItemText, active && styles.selectItemTextActive]}
                            numberOfLines={2}
                        >
                            {item.name}
                        </Text>
...
const ExpenditureSelect = React.memo(({
    items, selected, onSelect,
}: { items: ExpenditureItem[]; selected: number | null; onSelect: (id: number) => void }) => (
        ...
        {items.map((item) => {
            ...
            return (
                ...
                >
                    ...
                    <Text style={[styles.expItemText, active && styles.expItemTextActive]} numberOfLines={1}>
                        {item.name}
                    </Text>
                    <Text style={[styles.expItemSub, active && styles.expItemSubActive]} numberOfLines={1}>
...
export default function SpendScreen() {
    ...
    /* ── Fetches ── */
    const { data: walletData, isLoading: walletLoading } = useFetch<WalletResponse>({
        queryKey: ['employee-wallets', employeeId],
        endpoint: `master-data/employee-wallets?employee_id=${employeeId}`,
        enabled: !!employeeId,
    });

    const { data: itineraryData, isLoading: itineraryLoading } = useFetch<ItineraryResponse>({
        queryKey: ['itineraries', 'employee', employeeId],
        endpoint: `itinerary?employee_id=${employeeId}`,
        enabled: !!employeeId,
    });

    const { data: itemsData, isLoading: itemsLoading } = useFetch<ExpenditureResponse>({
        queryKey: ['expenditure-items'],
        endpoint: `umrah/expenditure_item?page=1&paginate=10&per_page=10`,
    });

    /* ── Derived ── */
    const defaultWalletId = useMemo(() => walletData?.data?.data?.[0]?.id ?? null, [walletData]);
    ...
    const isValid = defaultWalletId && selectedItinerary && selectedItem && amount && description;
    ...
    }, [isValid, defaultWalletId, selectedItinerary, selectedItem, amount, description, mutate]);
...
const styles = StyleSheet.create({
    ...
    currencyTag: {
        fontFamily: APP_FONT_FAMILY,
        fontSize: 13, color: PRIMARY, fontWeight: '500',
        marginRight: 8,
    },
    input: {
        flex: 1,
    ...
    loadingText: {
        marginTop: 12, fontFamily: APP_FONT_FAMILY,
        fontSize: 14, color: '#888',
    },
    emptyText: {
        fontFamily: APP_FONT_FAMILY, fontSize: 13,
        color: '#bbb', textAlign: 'center', paddingVertical: 8,
    },
});